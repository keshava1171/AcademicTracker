...
if ($_SERVER["REQUEST_METHOD"] == "POST" && isset($_POST['addUserForm'])) { 
    ...
    $student_class_id = filter_input(INPUT_POST, 'student_class', FILTER_VALIDATE_INT); 
        ...
        if (strpos($exceptionMessage, 'Duplicate entry') !== false) {
            if (strpos($exceptionMessage, "'username'") !== false) {
                ...
                $error_details = "Username '" . htmlspecialchars($dup_username) . "' already exists. Please choose a different one.";
                 ...
                 $dup_email = isset($email) ? $email : '[unknown]'; 
             ...
             error_log("User creation error: " . $exceptionMessage); 
             $error_details = "An unexpected error occurred during user creation. Please check logs or contact support."; 
         }
         $submitted_role = isset($_POST['role']) ? $_POST['role'] : 'teacher'; 
                                    ...
                                    <tr>
                                        ...
                                        <?php if ($active_tab === 'teacher'): ?>
                                            <td>
                                                ...
                                                echo htmlspecialchars(implode(", ", array_unique($classes_list))); 
                                        ...
                                        <?php elseif ($active_tab === 'student'): ?>
                                            <td>
                                                ...
                                                $class_name = 'N/A (Query Prepare Failed)'; 
                                                 if ($stmt_student_class) {
                                                     ...
                                                     if ($stmt_student_class->execute()) {
                                                         ...
                                                         if ($class = $student_class_result->fetch_assoc()) {
                                                             ...
                                                             $class_name = 'N/A (Class Not Found/Inactive)'; 
                                                         }
                                                     } else {
                                                         $class_name = 'N/A (Query Execute Failed: ' . $stmt_student_class->error . ')'; 
                                                     }
